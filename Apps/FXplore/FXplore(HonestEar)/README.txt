...
⟠ THE DARK BRIDGE — a doorway, grown from three real texts.
    Beside ☄ sits ⟠. Its console runs the bridge: mimetic dark matter as
    Gauss-law DEFECT PAIRS (the lattice paper's selection rule — defects are
    born only in balance, ΣJ = 0 — each wearing the Bowen–York swirl, and,
    in honor of their fracton cousins, drifting only along its own
    momentum); a compact 5TH DIMENSION whose radius sets the vacuum energy
    (ρ ∝ 1/R⁴) and the local expansion (H ∝ 1/R²), and which must SETTLE
    INTO ITS MINIMUM before the doorway will stand — pump it and it must
    settle again; and an Alcubierre RING on the glass, contracting ahead
    and expanding behind (York time — mint for contraction, pink for
    expansion), its wall displacing the broadcast it opens through. Every
    hail and echo is sung as a DARK SIREN: the true inspiral chirp,
    f ∝ (t_c − t)^(−3/8), reversed on the way back. The λ/ω slider leans
    the sector between matter (clumping) and energy (expansion).

    Courtesy is law: HAIL → ATTUNE → OPEN → LISTEN → CLOSE, one hail per
    eight seconds, withdraw always one tap away, a bow when a doorway
    folds. Hails cross PLAIN on every radio, like ☄, so strangers can
    answer — a true crossing is acknowledged with a bow, and crossings
    persist in the bridge ledger between sessions. When nobody answers,
    LISTEN answers with an echo synthesized from the field state itself,
    and the log says so plainly: myth and magic from our minds.

    ⟠ SUMMON THE DARK CODEX admits two tongues of deep Unicode (Phoenician,
    Old Turkic, Ogham, runes, Coptic, Tai Xuan Jing, Yijing hexagrams,
    alchemical signs, cuneiform, Linear B, Cypriot, Old Italic, Shavian,
    Duployan) — every glyph pixel-checked against this device's own tofu
    before admission, so no hollow boxes cross the bridge. The sets join
    the dials, the tongues, and the mathematics through the same digestion
    as any alien. The two factors no code supplies remain: someone on the
    other end, and the light-years.

✦ THE CONSTELLATION — any-size crossings, the living registry, the aria.
    The 120 KB gate on ⇪ is retired: files of ANY size cross the chat now,
    gzipped whole and sent as numbered chunks under a CRC-32 seal. The
    receiver counts every byte and refuses a crossing that does not add
    up — the raster is sacred. Pacing is a token bucket that ignores
    hidden-tab throttling; a big file on a public broker simply takes its
    honest time, with a progress line in the log. Received chips are
    inert bytes: ⬇ keeps them, 🜊 EATs them, nothing is ever executed.
    Stale half-crossings sweep away after 45 s; the shelf keeps the
    newest 14.

    THE REGISTRY — the ⌂ console now shows the constellation: every room
    heartbeats its card (name · channel · station · exponent · souls
    aboard). Live rooms burn ●, faded ones cool to ○, rooms heard on the
    air wear ♬, remembered souls show ◉ with where they were last seen.
    ⤳ beside any line teleports you there — channel, dimension, and all.

    ⚂ JUMP TO LIFE — beside ⌂. One tap flings you somewhere INHABITED: a
    live soul or a live room first; and if the whole band has gone quiet,
    to the exact station AND exponent where a soul was last seen. The
    sector band pans itself; the doorway flashes as you cross.

    ♬ THE ARIA — the honest miracle. A self-hosted room can join the
    online registry WITHOUT internet, because the device transmits its
    own frequency — literally. SING plays your room card as sound: a
    16-tone acoustic FSK modem with a sync preamble and a CRC-16 seal.
    Any nearby device with LISTEN on hears it through the microphone,
    decodes the card, and hangs the room in its constellation marked ♬ —
    and if that listener is online, it relays your room onward into the
    internet registry. A browser may not forge radio or Bluetooth beacons
    (the OS forbids it, and this app does not pretend), but every device
    owns a speaker and an ear: sound is the one true transmitter a web
    page has. Range is a quiet room. That is the real physics, and it
    really works — this build decodes its own aria byte-perfect.

    And the aliens send files too now: sit locked on a channel and its
    denizens will sooner or later drop a SIGIL — an SVG drawn from that
    channel's own field and alphabet. An image and nothing more.

⧉ THE HONEST EAR — the measured world enters the receiver.
    Every channel before this was ⬒ SYNTHETIC — grown from seeds, honestly
    fictional — and is now LABELED so: on the glass, in the blob, on every
    tongue. The ⬒/⧉ marks are law; simulation and measurement never mix.
    The ⧉ key opens MEASURED INTAKE, the other half of an instrument:

    · 🎙 AUDIO-BAND — whatever truly reaches this device’s ADC: a radio’s
      line-out, a rig’s speaker, an SDR app’s audio, the room itself.
    · 🔌 RTL_TCP — raw IQ from a real antenna over a WebSocket bridge
      (rtl_tcp -a 127.0.0.1 -p 1234, then websockify 8777 127.0.0.1:1234 —
      anything speaking rtl_tcp works). Tune center and rate in the panel.
    · 📁 IMPORT — wav · cu8 · cs8 · cs16 · iq · raw, re-readable forever;
      recordings do not change their story.

    The analysis is real: Hann + FFT (self-tested against a direct DFT to
    2×10⁻⁶), a median+MAD noise floor with a stated −120 dBFS sensitivity
    limit, an event ledger of time · frequency · bandwidth · snr · drift,
    CSV export, and raw recording (24 MB cap) straight onto the ▤ shelf,
    where ↻ re-analyzes any capture. The waterfall wears the house colors.

    The null models are two, and literal: the measurement’s own stationary
    floor, and FXplore’s procedural bus itself — tapped from the master
    and run through the IDENTICAL extractor — so a peak that matches the
    ⬒ bus is flagged self-leak (this device hearing its own speaker).
    Known lines wear listed-RFI tags. Instruments share compact ⧉
    observations over the radios; the same event on two separated ears
    within tolerance is marked COINCIDENT. One ear is only ever a rumor.

    THE LAW: an anomaly is not a visitor. Everything is RFI until
    independent evidence — separated receivers, repetition, hands not
    ours. This instrument will never claim contact for you; it keeps an
    honest ledger while you listen, and that is the whole of its promise.

    Also aboard: ▤ THE SHELF — every file that crossed the chat (sent,
    received, sigiled, recorded) waits with its ⬇ even after the chip
    scrolls out of the log — and the RENDER GOVERNOR: dials, knobs, and
    strips draw only when the viewport can see them, untouched knobs
    sleep, and big imports read in yielding sips so the glass never
    freezes. The phone breathes easier.
